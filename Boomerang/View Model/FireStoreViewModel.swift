...
extension FireStoreViewModel {    
    func uploadProduct(images: [UIImage], POST_CONTENT: String, POST_TITLE: String, PRICE: Int, OWNER_ID: String, PRODUCT_NAME: String, PRODUCT_TYPE: String) {
                    ...
                    FireStoreService.downloadURL(ref: storageReference)
                        .sink(receiveCompletion: { completion in
                            ...
                            if images.count == images_map.count {
                                let product: Dictionary<String, Any> = ["IMAGES_MAP": images_map, "AVAILABILITY": true, "LOCATION": "동백동", "PRODUCT_NAME": PRODUCT_NAME, "PRODUCT_TYPE": PRODUCT_TYPE, "POST_CONTENT": POST_CONTENT, "POST_TITLE": POST_TITLE, "PRICE": PRICE, "OWNER_ID": OWNER_ID, "TIMESTAMP": Date()]
                                
                                FireStoreService.uploadDocument(newProduct: product)
                                    .sink(receiveCompletion: { completion in
                                        switch completion {
                                        case .finished:
    ...
    func deleteProduct(target: Product) {
        FireStoreService.deleteDocument(id: target.id)
            .sink(receiveCompletion: { completion in
                switch completion {
                case .finished:
